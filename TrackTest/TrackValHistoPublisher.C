...
/*
root -b -q TrackValHistoPublisher.C\(\"trackValid.root\",\"stripTripletStepHP\",\"trackValid.root\",\"stripPairStepHP\",0.2,0.7,0.1\)
root -b -q TrackValHistoPublisher.C\(\"seedValid.root\",\"stripTripletStepSeeds\",\"seedValid.root\",\"pixelLessStepSeeds\",0.2,1.0,0.1\)
--------------------------------
for step in {pixelLessStepHP,tobTecStepHP,stripPairStepHP}; do root -b -q  TrackValHistoPublisher.C\(\"trackValid.root\",\"stripTripletStepHP\",\"trackValid.root\",\"${step}\",0.2,0.7,0.1\); done
for step in {pixelLessStepSeeds,tobTecStepSeeds}; do root -b -q  TrackValHistoPublisher.C\(\"seedValid.root\",\"stripTripletStepSeeds\",\"seedValid.root\",\"${step}\",0.2,1.1,0.1\); done
--------------------------------
*/
//cp -r $PWD ~/www/dummyplots/stripTripletSeeding/; rm ~/www/dummyplots/stripTripletSeeding/*/*.root

void TrackValHistoPublisher(char* newFile="trackValid.root",char* newLabel="stripTripletStepHP", 
			    char* refFile="trackValid.root",char* refLabel="stripPairStepHP",
			    float MAXEFF=0.2,float MAXFAKE=0.7,float MAXDUPL=0.1)
{
 ...
 TString dir = TString(newLabel)+"_vs_"+TString(refLabel);
...
//   sh2->GetYaxis()->SetRangeUser(0.,.70);
   ...
   rh1->GetYaxis()->SetRangeUser(0,24);
   sh1->GetYaxis()->SetRangeUser(0,24);
   rh1->GetXaxis()->SetTitle("#eta");
   ...
   rh3->GetXaxis()->SetRangeUser(0,50);
   sh3->GetXaxis()->SetRangeUser(0,50);
   rh3->GetXaxis()->SetTitle("p_{t}");
   rh4->GetXaxis()->SetRangeUser(0,50);
   sh4->GetXaxis()->SetRangeUser(0,50);
   rh4->GetXaxis()->SetTitle("p_{t}");
