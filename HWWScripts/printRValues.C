...
void printRValues(bool doMVA=0, bool useMC=1){
  ...
  if (nmasses>6 && njetbins==3 && masses[0]==0) {
    ...
    if (!doMVA) out << Form("Double_t RoutinStatError(Int_t mH, Int_t jetBin) {\n");
    else  out << Form("Double_t RoutinBDTStatError(Int_t mH, Int_t jetBin) {\n");
    out << Form("  Int_t mHiggs[%i] = {",nmasses-1);
    for (int j=1;j<nmasses-1;++j) out << Form("%i,",masses[j]);
    out << Form("%i};\n",masses[nmasses-1]);
    out << Form("  Double_t RoutinWWPreselectionStatError[3] = { %7.5f,%7.5f,%7.5f  };\n",vst0j[0],vst1j[0],vst2j[0]);
    out << Form("  Double_t RoutinHiggsSelectionStatError[3][%i] = { \n",nmasses-1);
    out << Form("    { ");
    ...
    out << Form("%7.5f} }; \n",vst2j[nmasses-1]);
    out << Form("  if(mH == 0) return RoutinWWPreselectionStatError[jetBin];\n");
    out << Form("  Int_t massIndex = -1;\n");
    out << Form("  for (UInt_t m=0; m < %i ; ++m) {\n",nmasses);
    out << Form("    if (mH == mHiggs[m]) massIndex = m;\n");
    out << Form("  }\n");
    out << Form("  if (massIndex >= 0) {\n");
    out << Form("    return RoutinHiggsSelectionStatError[jetBin][massIndex];\n");
    out << Form("  } else {\n");
    out << Form("    return RoutinWWPreselectionStatError[jetBin];\n");
    out << Form("  }\n");
    out << Form("}\n");
    
    if (!doMVA) out << Form("Double_t RoutinSystError(Int_t mH, Int_t jetBin) {\n");
    else  out << Form("Double_t RoutinBDTSystError(Int_t mH, Int_t jetBin) {\n");
    out << Form("  Int_t mHiggs[%i] = {",nmasses-1);
    for (int j=1;j<nmasses-1;++j) out << Form("%i,",masses[j]);
    out << Form("%i};\n",masses[nmasses-1]);
    out << Form("  Double_t RoutinWWPreselectionSystError[3] = { %7.5f,%7.5f,%7.5f  };\n",vsy0j[0],vsy1j[0],vsy2j[0]);
    out << Form("  Double_t RoutinHiggsSelectionSystError[3][%i] = { \n",nmasses-1);
    out << Form("    { ");
    ...
    out << Form("%7.5f} }; \n",vsy2j[nmasses-1]);
    out << Form("  if(mH == 0) return RoutinWWPreselectionSystError[jetBin];\n");
    out << Form("  Int_t massIndex = -1;\n");
    out << Form("  for (UInt_t m=0; m < %i ; ++m) {\n",nmasses);
    out << Form("    if (mH == mHiggs[m]) massIndex = m;\n");
    out << Form("  }\n");
    out << Form("  if (massIndex >= 0) {\n");
    out << Form("    return RoutinHiggsSelectionSystError[jetBin][massIndex];\n");
    out << Form("  } else {\n");
    out << Form("    return RoutinWWPreselectionSystError[jetBin];\n");
    out << Form("  }\n");
    out << Form("}\n");
