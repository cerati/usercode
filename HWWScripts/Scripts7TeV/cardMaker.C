...
void cardMaker(float lumi, int mass, unsigned int njets, TString fs, TString mode, bool saveFile=false) {
  ...
  if (mode=="cut") wwSF = make_pair<float, float>(WWBkgScaleFactorCutBased(massForWW,njetsForWW), WWBkgScaleFactorCutBased(massForWW,njetsForWW)*(WWBkgScaleFactorKappaCutBased(massForWW,njetsForWW)-1.));
  else if (mode=="shape") wwSF = make_pair<float, float>(WWBkgScaleFactorMVA(massForWW,njetsForWW), WWBkgScaleFactorMVA(massForWW,njetsForWW)*(WWBkgScaleFactorKappaMVA(massForWW,njetsForWW)-1.));
  /*
  if (mass>200 && doMVA==0) {
  ...
  */
  ...
  pair<float, float> zz = getYield(dir+"zz_py", wwSelNoMet, veto, mass, njets, sigreg+"=notZ="+fs, lumi, useJson, applyEff, doFake, doPUw);
  ...
  pair<float, float> pzz = getYield(dir+"zz_py", wwSelNoMet, veto, mass, njets, sigreg+fs+"=fromZ=", lumi, useJson, applyEff, doFake, doPUw);
  ...
  //cout << mode << " " << njets  << " " << fs << " mc=" << dyll.first << " sf=" << dySF.first << "+/-" <<  dySF.second 
  //     << " yield=" << dyY << " VZ=" << pzz.first+pwz.first << " k=" << dyDDK << endl;
  ...
  if (makeTable) {
    ...
    if (mass>190) {
      out << Form("%-40s %5s   1.022   1.022   1.022   1.022   1.022   1.022   1.022     -       -       -     1.022   1.022\n","lumi_7TeV","lnN");
    } else {
      out << Form("%-40s %5s   1.022   1.022   1.022   1.022     -       -     1.022     -       -       -     1.022   1.022\n","lumi_7TeV","lnN");
    }
    if (mode=="shape" && doResEffSyst) {
      out << Form("%-40s %5s   %5s   %5s   1.000   1.000   1.000   1.000   1.000     -       -       -     %5s   %5s \n","CMS_hww_MVALepEffBounding","shape",
		  zhww.first>0?"1.000":"  -  ",whww.first>0?"1.000":"  -  ",(wgamma.first+zgamma.first+wg3l.first)>0?"1.000":"  -  ",dytt.first>0?"1.000":"  -  ");//why no top???
      out << Form("%-40s %5s   %5s   %5s   1.000   1.000   1.000   1.000   1.000   1.000     -       -     %5s   %5s \n","CMS_hww_MVALepResBounding","shape",
		  zhww.first>0?"1.000":"  -  ",whww.first>0?"1.000":"  -  ",(wgamma.first+zgamma.first+wg3l.first)>0?"1.000":"  -  ",dytt.first>0?"1.000":"  -  ");
    ...
    out << Form("%-40s %5s     -       -       -       -     %5.3f   %5.3f     -       -       -       -       -       -  \n", (mass<=190||mode=="shape") ? Form("CMS_hww_%ij_WW_7TeV",njets) : "CMS_hww_WW","lnN",
    ...
    if (mode=="cut") {
      ...
      out << Form("%-40s %5s     -       -       -       -       -       -       -       -       -     %5.3f     -       -  \n",Form("CMS_hww%s_stat_%ij_Wjets_7TeV",fs.Data(),njets),"lnN",
      ...
      if (fs=="sf") out << Form("%-40s %5s     -       -       -       -       -       -       -       -     1.000     -       -       -  \n",Form("CMS_hww%s_%ij_MVAZBounding",fs.Data(),njets),"shape");      
      //these are from MC so are in common for 7TeV and 7TeV
	...
	out << Form("%-40s %5s     -       -       -       -       -       -       -       -     1.000     -       -       -  \n",Form("CMS_hww%s_%ij_MVAZjetsStatBounding_7TeV",fs.Data(),njets),"shape");      
      if (wjets.first>0.)
	out << Form("%-40s %5s     -       -       -       -       -       -       -       -       -     1.000     -       -  \n",Form("CMS_hww%s_%ij_MVAWjetsStatBounding_7TeV",fs.Data(),njets),"shape");      
