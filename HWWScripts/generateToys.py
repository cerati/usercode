...
seed = 1
for mass in masses:
    os.chdir(os.getcwd()+'/cards/'+str(mass)+'/')
    for cms in cmss:
        for njets in jetbins:
            for fs in fstates:
                for type in types:
                    ...
                    os.system('../../LandS/test/lands.exe -d '+card+'.txt -M Hybrid  -m '+str(mass)+' --minuitSTRATEGY 0  --bWriteToys 1 -n \"'+card+'\" --nToysForCLsb 1000 --nToysForCLb 1 --singlePoint 1 --seed '+str(seed)+'  -rMin 0 -rMax 5  --freq >& toy_'+card+'.log')
                    seed=seed+1
    os.chdir(os.getcwd()+'/../../')
