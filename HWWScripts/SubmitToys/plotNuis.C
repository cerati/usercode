...
void plotNuisSingle(TString inj,int jet, int mH, TString dir, TString ana, int ntoys) {
  ...
  for(int i=0; i<ntoys; i++) {
    ...
    for(int j=0; j<parlist.getSize(); j++) {
      ...
      float central = ((RooRealVar&)parlist[j]).getVal();
      if (name_tstr=="r")  {
	name_tstr+=Form("_%ij",jet);
	central=central-1.;
      }
      if (name_tstr.Contains("0j")==0 && name_tstr.Contains("1j")==0 ) {
	name_tstr+=Form("_%ij",jet);
      }
      float uncert = ((RooRealVar&)parlist[j]).getError();
      createFillTH1F(fout,TString(name_tstr+"_pull").Data(),TString(name_tstr+"_pull").Data(),100,-5,5,"(nuis_{fit} - <nuis_{in}>)/#sigma_{nuis,fit}","toys/bin",central/uncert);
      createFillTH1F(fout,TString(name_tstr+"_uncert").Data(),TString(name_tstr+"_uncert").Data(),100,0,2,"#sigma_{nuis,fit}/#sigma_{nuis,in}","toys/bin",uncert);
      createFillTH1F(fout,TString(name_tstr+"_central").Data(),TString(name_tstr+"_central").Data(),100,-5,5,"(nuis_{fit} - <nuis_{in}>)/#sigma_{nuis,in}","toys/bin",central);
    }
